...
<div class="col-md-9">
	...
	<form role="form" method="POST">
		...
		<table class="table table-bordered" id="my-tbody">
				...
				<td>
					...
					%> <select name="largeCategory" id="jobGroup">
						...
						<c:forEach items="${jobgroupList}" var="JobGroupVO">
							...
							<option value=<%=jobgroupCounter%>>${JobGroupVO.jobgroup}</option>

						</c:forEach>
				</select> <select id="subjobGroup" name="jobgroupid">
				</select> <input type="button" id="btn1" value="추가" /> <a id="jobgroupZone">
				</a>
				</td>
			</tr>
			<tr>
				<th>근무지역</th>
				<td id=""><select name="rgbid" id="region">
						<option value="">근무지역</option>
						<c:forEach items="${regionList}" var="RegionVO">
							<option value="${RegionVO.rgbid}">${RegionVO.rgbname}</option>
						</c:forEach>
				</select> <select name="rgsid" id="subRegion">
				</select></td>
			</tr>
			<tr>
				<th>담당업무</th>
			...
			<tr>
				<th>접수방법</th>
				<td><input type="radio" name="acceptmethod">홈페이지접수
					&nbsp; <input type="radio" name="sbm">즉시지원</td>
			</tr>
		</table>
...
<script>
	...
	function SubJobGroup(largeNum) {
		$.getJSON("/companyAjax/jobGroup/" + largeNum, function(data) {
			...
			$(data).each(
					function() {
						str += "<option value="+this.id+">" + this.jobgroup
								+ "</option>";
					});
...
<script>
	...
	function jobgroupPlus() {
		$.getJSON("/companyAjax/jobGroup", function(data) {
			var str = "";
			$(data).each(function() {
